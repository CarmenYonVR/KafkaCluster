...
cd kafka_2.13-4.2.0
KAFKA_CLUSTER_ID=$(aws secretsmanager get-secret-value --region ${REGION} --secret-id ${KAFKA_SECRET_NAME} --query SecretString --output text | jq -r ".cluster_id")
BROKER0_ID=$(aws secretsmanager get-secret-value --region ${REGION} --secret-id ${KAFKA_SECRET_NAME} --query SecretString --output text | jq -r ".broker0_id")
BROKER1_ID=$(aws secretsmanager get-secret-value --region ${REGION} --secret-id ${KAFKA_SECRET_NAME} --query SecretString --output text | jq -r ".broker1_id")
BROKER2_ID=$(aws secretsmanager get-secret-value --region ${REGION} --secret-id ${KAFKA_SECRET_NAME} --query SecretString --output text | jq -r ".broker2_id")
